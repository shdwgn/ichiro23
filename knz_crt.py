...
hx=("""  [bold green1]DEVELOPER[medium_purple1]   ⟩[cyan][bold] Ichiro
  ...
  [bold green1]GIRLFRIEND[medium_purple1]  ⟩[cyan][bold] JANNA SAMSON
  [bold green1]GITHUB[medium_purple1]      ⟩[bright_yellow] shdwgn.git
...
def confirm_id(mail,uid,otp,data,ses):
    try:
        ...
        if "checkpoint" in str(response.url):
            ...
            hx=("[bold green]"+cok)
            Ichiroa = Panel.fit("[green] LOGIN SUCCESS",style="bold violet")
            Ichirob = Panel("[green] "+uid, title="[bold green]UID",width=30,padding=0,style="bold violet")
            Ichiroc = Panel(f"[green] {password}", title="[bold green]PASS",width=30,padding=0,style="bold violet")
            Ichiroe = Columns([Bryxa])
            Ichirof = Columns([Bryxb, Bryxc])
    ...
    if id:
        ...
        if 'Locked' in check:
            ...
            if code:
                ...
                hx=("[bold green]"+token)
                Ichiroa = Panel.fit("[green] LOGIN SUCCESS",style="bold violet")
                Ichirob = Panel("[green] "+id, title="[bold green]UID",width=30,padding=0,style="bold violet")
                Ichiroc = Panel(f"[bold green] {password}", title="[bold green]PASS",width=30,padding=0,style="bold violet")
                Ichiroe = Columns([Ichiroa])
                Bryxf = Columns([Ichirob, Ichiroc])
                c=Tree(":file_folder:",guide_style="bold green_yellow")
                c.add(Ichiroe)
                c.add(Ichirof)
                c.add(Panel(hx,style="bold violet"))
