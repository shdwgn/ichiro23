...
def fake_email_name(): first = Faker().first_name() last = Faker().last_name() return first,last #──────────────{ FAKE EMAIL }──────────────# def fake_email(): name = " ".join(fake_email_name()).replace(' ', '') namepassword = f'{name}' return namepassword #──────────────{ EMAIL ADDRESS }──────────────# def GetEmails(): nam1 = fake_email() name = f'{nam1}' domain = random.choice(['gmail.com','yahoo.com','hotmail.com','gonetor.com']) nu = str(random.randrange(10000, 100000)) nope = f'{name}@{domain}' return nope #──────────────{ PHONE NUMBER }──────────────# def GetPhone(): na = random.choice(['77', '78', '59']) ni = str(random.randrange(1000, 10000)) nu = str(random.randrange(10000, 100000)) nope = '+639%s%s%s' % (na, ni, nu) return nope #──────────────{ OK/CP }──────────────# oks = [] cps = [] n=0 lim=0 tf=0 threadcount = 0 proxylist = [] acclist = [] alreadychecked = [] checkerqueue = [] live = 0 dead = 0 checkpoint = 0 fullsize = 0 #──────────────{ FAKE NAME }──────────────# def fake_name(): first = Faker().first_name() last = Faker().last_name() return first,last

def fake_namee(): first = Faker("id_ID").first_name() last = Faker("id_ID").last_name() return first,last #──────────────{ FAKE NAME }──────────────# def fake_name(): first = Faker().first_name() last = Faker().last_name() return first,last

def fake_namee(): first = Faker("id_ID").first_name() last = Faker("id_ID").last_name() return first,last #──────────────{ FAKE PASSWORD }──────────────# def fake_password(): name = " ".join(fake_name()).replace(' ', '') namepassword = f'{name}{str(random.randrange(1000,10000))}' return namepassword

def fakee_password(): nama = " ".join(fake_namee()).replace(' ', '') namapassword = f'{nama}{str(random.randrange(1000,10000))}' return namapassword #──────────────{ FAKE EMAIL }──────────────# def fake_email(): name = " ".join(fake_name()).replace(' ', '') nameemail = f'{name}{str(random.randrange(1000,10000))}@yopmail.com' return nameemail

def fakee_email(): nama = " ".join(fake_namee()).replace(' ', '') namaemail = f'{nama}{str(random.randrange(1000,10000))}@yopmail.com' return namaemail #──────────────{ SPACE SYSTEM }──────────────# def space(): print("\n") #──────────────{ CLEAR }──────────────# def clear(): os.system("clear") #──────────────{ STATUS }──────────────# status = f"ACTIVE" #──────────────{ GREETING }──────────────# def greet_ph(): bd_timezone = pytz.timezone("Asia/Manila") bd_time = datetime.now(bd_timezone).hour if 5 <= bd_time < 12: return "GOOD MORNING" elif 12 <= bd_time < 18: return "GOOD AFTERNOON" else: return "GOOD NIGHT" #──────────────{ COUNTRY }──────────────# def countryX(): country = "PHILIPPINES" return country #──────────────{ SIM NAME }──────────────# simX = subprocess.check_output('getprop gsm.operator.alpha',shell=True).decode('utf-8').replace('\n','').replace(',',f'/') #──────────────{ CALL AREA }──────────────# country = countryX() greeting = greet_ph() current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S") #──────────────{ LOGO }──────────────# logo=(f"""

[green1] ██╗ █████╗ ██╗ ██╗██████╗ █████╗ [spring_green2] ██║██╔══██╗██║ ██║██╔══██╗██╔══██╗ [medium_spring_green] ██║███████║███████║██████╔╝███████║ [cyan2]██ ██║██╔══██║██╔══██║██╔══██╗██╔══██║ [cyan1]╚█████╔╝██║ ██║██║ ██║██║ ██║██║ ██║ [cyan1] ╚════╝ ╚═╝ ╚═╝╚═╝ ╚═╝╚═╝ ╚═╝╚═╝ ╚═╝ [cyan][bold]VERSION/1.0 [green_yellow]MY [dark_olive_gre]SYSTEM[pale_green1] IS[dark_sea_green…] DIFFERENT BROTHER """) ll=str([hari,tanggal]) hx=(f""" [bold green1]OWNER/ADMIN[medium_purple1] ➙ [cyan][bold]JAHRA [bold green1]TOOL TYPE[medium_purple1] ➙ [green][bold]AUTOMATIC CREATE FACEBOOK/PAID TOOLS [bold green1]STATUS[medium_purple1] ➙ [green][bold]PREMIUM [bold green1]TODAYS[medium_purple1] ➙ [green][bold]{date} [bold green1]GREET[medium_purple1] ➙ [green][bold]{greeting} [bold green1]SIM[medium_purple1] ➙ [green][bold]{simX} [bold green1]COUNTRY[medium_purple1] ➙ [green][bold]{country}""") #──────────────{ BANNER/CLEAR }──────────────# def banner(): os.system("clear") print(Panel(logo,subtitle="[bold red]❏ [bright_yellow]❏ [green1]❏",subtitle_align='left',title="[bold red]❏ [bright_yellow]❏ [green1]❏",title_align='right',width=102,padding=0,style="bold purple")) print(Panel(hx,width=100,padding=0,style="bold purple")) #──────────────{ MAIN/MENU }──────────────# def AUTO_BRYX(): banner() a=(Panel(""" [green_yellow][[bold cyan1]1/A[green_yellow]][bold green] AUTO CREATE FACEBOOK\n [green_yellow][[bold cyan1]2/B[green_yellow]][bold green] TOKEN LIVE/CHECKPOINT/DEAD CHECKER\n [green_yellow][[bold cyan1]3/C[green_yellow]][bold green] GUARD PROFILE\n [green_yellow][[bold cyan1]4/D[green_yellow]][bold green] AUTO 2F\n [green_yellow][[bold cyan1]5/E[green_yellow]][bold green] CONTACTED OWNER/ADMIN\n [green_yellow][[bold cyan1]6/F[green_yellow]][bold red] EXIT TOOL """,title="[reverse purple] TOOL MENU",style="bold purple")) print(Panel(a,subtitle="[bold purple]┌─",subtitle_align='left',style="bold purple")) Bryx = Console().input(" [bold purple]└──> ") if Bryx in ["a","A","1","01"]: menu() elif Bryx in ["b","B","2","02"]: print(Panel(f" [bold white]Wait for update",style="bold purple")) elif Bryx in ["c","C","3","03"]: guardon() elif Bryx in ["d","D","4","04"]: auto2f() elif Bryx in ["e","E","5","05"]: os.system("xdg-open https://www.facebook.com/aly.smith.50159") time.sleep(2);AUTO_BRYX() elif Bryx in ["f","F","6","06"]:exit() else:AUTO_BRYX() #──────────────{ RESULTS }──────────────# def results(): print(Panel(f"""[bold white] FINAL RESULTS\n SUCCESS : [bold green]{len(oks)}[/]\n BAD : [bold red]{len(cps)}[/]\n RESULTS ARE SAVED TO THE RESULTS FOLDER""",subtitle="[bold red]❏ [bright_yellow]❏ [green1]❏",subtitle_align='left',title="[bold red]❏ [bright_yellow]❏ [green1]❏",title_align='right',width=102,padding=0,style="bold purple")) #──────────────{ MAIN/MENU }──────────────# def menu(): from datetime import date fake = Faker() fakee = Faker("id_ID") banner() a=(Panel(""" [green_yellow][[bold cyan1]1/A[green_yellow]][bold green] DATA 1\n [green_yellow][[bold cyan1]2/B[green_yellow]][bold green] DATA 2\n [green_yellow][[bold cyan1]3/C[green_yellow]][bold green] DATA 3\n [green_yellow][[bold cyan1]4/D[green_yellow]][bold green] WIFI 5\n [green_yellow][[bold cyan1]5/E[green_yellow]][bold green] WIFI 6 """,title="[reverse purple] ALL NETWORK ",style="bold purple")) print(Panel(a,subtitle="[bold purple]┌─",subtitle_align='left',style="bold purple")) mthd = console.input(" [bold purple]└──> ") banner() print(Panel(" [bold green]ENTER HOW MANY ACC", subtitle='[bold purple]╭─────',subtitle_align='left',style="bold purple")) num_accounts = int(console.input(" [bold purple]└──> ")) banner() a=(Panel(""" [green_yellow][[bold cyan1]1/A[green_yellow]][bold green] PHILIPPINES NAMES\n [green_yellow][[bold cyan1]2/B[green_yellow]][bold green] INDONESIA NAMES """,title="[reverse purple] NAME ",style="bold purple")) print(Panel(a,subtitle="[bold purple]┌─",subtitle_align='left',style="bold purple")) bryxname = console.input(" [bold purple]└──> ") banner() a=(Panel(""" [green_yellow][[bold cyan1]1/A[green_yellow]][bold green] DATE OF BIRTH """,title="[reverse purple] BIRTHDAY ",style="bold purple")) print(Panel(a,subtitle="[bold purple]┌─",subtitle_align='left',style="bold purple")) bryxbirthday = console.input(" [bold purple]└──> ") banner() a=(Panel(""" [green_yellow][[bold cyan1]1/A[green_yellow]][bold green] MALE/FEMALE """,title="[reverse purple] GENDER ",style="bold purple")) print(Panel(a,subtitle="[bold purple]┌─",subtitle_align='left',style="bold purple")) bryxgender = console.input(" [bold purple]└──> ") banner() a=(Panel(""" [green_yellow][[bold cyan1]1/A[green_yellow]][bold green] FAKE EMAIL PHILIPPINES\n [green_yellow][[bold cyan1]2/B[green_yellow]][bold green] FAKE EMAIL INDONESIA """,title="[reverse purple] EMAIL ",style="bold purple")) print(Panel(a,subtitle="[bold purple]┌─",subtitle_align='left',style="bold purple")) bryxemail = console.input(" [bold purple]└──> ") banner() a=(Panel(""" [green_yellow][[bold cyan1]1/A[green_yellow]][bold green] DEFAULT PHILIPPINES PASSWORD\n [green_yellow][[bold cyan1]2/B[green_yellow]][bold green] DEFAULT INDONESIA PASSWORD """,title="[reverse purple] PASSWORD ",style="bold purple")) print(Panel(a,subtitle="[bold purple]┌─",subtitle_align='left',style="bold purple")) bryxpassword = console.input(" [bold purple]└──> ") banner() print(Panel(f" [bold green]ACCOUNT CREATING STARTED",style="bold purple")) print(Panel(f" [bold green]IF NO RESULT ON/OFF AIRPLANE MODE OR VPN 1.1.1.1",style="bold purple")) for _ in range(num_accounts):
birthdate = fake.date_of_birth(minimum_age=18, maximum_age=90) today = date.today() age = today.year - birthdate.year - ((today.month, today.day) < (birthdate.month, birthdate.day)) if bryxname in ["a","A","1","01"]: first_name = fake.first_name() last_name = fake.last_name() elif bryxname in ["b","B","2","02"]: first_name = fakee.first_name_female() last_name = fakee.last_name_female() if bryxbirthday in ["a","A","1","01"]: birthday = fake.date_of_birth(minimum_age=18, maximum_age=90) if bryxgender in ["a","A","1","01"]: gender = random.choice(["M", "F"]) if bryxemail in ["a","A","1","01"]: email = fake_email() elif bryxemail in ["b","B","2","02"]: email = fakee_email() if bryxpassword in ["a","A","1","01"]: password = fake_password() elif bryxpassword in ["b","B","2","02"]: password = fakee_password() if mthd in ["a","A","1","01"]: register_facebook_account1(password, first_name, last_name, birthday, age, gender, email) elif mthd in ["b","B","2","02"]: register_facebook_account2(password, first_name, last_name, birthday, age, gender, email) elif mthd in ["c","C","3","03"]: register_facebook_account3(password, first_name, last_name, birthday, age, gender, email) elif mthd in ["d","D","4","04"]: register_facebook_account4(password, first_name, last_name, birthday, age, gender, email) elif mthd in ["e","E","5","05"]: register_facebook_account5(password, first_name, last_name, birthday, age, gender, email) results() #──────────────{ LOCK ID/LIVE ID }──────────────# def lock(id): sessionx=requests.Session() urlx=f'https://www.facebook.com/p/{id}' req=bxx(sessionx.get(urlx).content,'html.parser') tx=req.find('title').text if tx =='Facebook': return('LOCK') else: return('LIVE') #──────────────{ FB LINK }──────────────# def generate_random_fb_url(): number_part = id if random.choice([True, False]): return f"https://www.facebook.com/profile.php?id=1000{number_part}" else: pass #──────────────{ GENERATE RANDOM STRING }──────────────# def generate_random_string(length): return ''.join(random.choices(string.ascii_letters + string.digits, k=length)) #──────────────{ AUTO CREATE METHOD 1 }──────────────# def register_facebook_account1(password, first_name, last_name, birthday, age, gender, email): session = requests.Session() api_key = '882a8490361da98702bf97a021ddc14d' secret = '62f8ce9f74b12f84c123cc23437a4a32' accessToken = '350685531728|62f8ce9f74b12f84c123cc23437a4a32' req = { 'api_key': api_key, 'attempt_login': True, 'birthday': birthday.strftime('%Y-%m-%d'), 'client_country_code': 'US', 'fb_api_caller_class': 'com.facebook.registration.protocol.RegisterAccountMethod', 'fb_api_req_friendly_name': 'registerAccount', 'firstname': first_name, 'format': 'json', 'gender': gender, 'lastname': last_name, 'email': email, 'locale': 'en_US', 'method': 'user.register', 'password': password, 'reg_instance': generate_random_string(32), 'return_multiple_errors': True } sorted_req = sorted(req.items(), key=lambda x: x[0]) sig = ''.join(f'{k}={v}' for k, v in sorted_req) ensig = hashlib.md5((sig + secret).encode()).hexdigest() req['sig'] = ensig api_url = 'https://b-api.facebook.com/method/user.register' headers = { 'User-Agent': ax(), } proxy = get_random_proxy() response = requests.post(api_url, data=req, headers=headers, proxies=proxy, timeout=30) reg = response.json() id = reg.get('new_user_id') token = reg.get('session_info', {}).get('access_token') url = generate_random_fb_url() if id: check = lock(id) if 'Locked' in check: os.system('espeak -a 300 "SHIT, DADDY, FUCK, FUCK, FUCK, FUCK, I, GET, CUM, DISABLED CP"') cps.append(id) else: print(Panel(f"[bold white] CREATING ACCOUNT PLEASE WAIT...",style="bold purple")) dn() print(Panel(f"[bold white] REGISTERING ACCOUNT",style="bold purple")) dn() print(Panel(f"[bold white] FILLING UP CREDENTIALS",style="bold purple")) dn() print(Panel(f"[bold white] CHECKING ACCOUNT LIVE OR NOT",style="bold purple")) dn() print(Panel(f"[bold white] FIRSTNAME",style="bold purple")) dn() print(Panel(" [bold white]FIRSTNAME", subtitle='[bold purple]╭─────',subtitle_align='left',style="bold purple")) print(f" [bold purple]└──> [bold green]{first_name}") dn() print(Panel(f"[bold white] LASTNAME",style="bold purple")) dn() print(Panel(" [bold white]LASTNAME", subtitle='[bold purple]╭─────',subtitle_align='left',style="bold purple")) print(f" [bold purple]└──> [bold green]{last_name}") dn() print(Panel(f"[bold white] AGE",style="bold purple")) dn() print(Panel(" [bold white]AGE", subtitle='[bold purple]╭─────',subtitle_align='left',style="bold purple")) print(f" [bold purple]└──> [bold green]{age}") dn() print(Panel(f"[bold white] BIRTHDAY",style="bold purple")) dn() print(Panel(" [bold white]BIRTHDAY", subtitle='[bold purple]╭─────',subtitle_align='left',style="bold purple")) print(f" [bold purple]└──> [bold green]{birthday}") dn() print(Panel(f"[bold white] GENDER",style="bold purple")) dn() print(Panel(" [bold white]GENDER", subtitle='[bold purple]╭─────',subtitle_align='left',style="bold purple")) print(f" [bold purple]└──> [bold green]{gender}") dn() print(Panel(f"[bold white] EMAIL ADDRESS",style="bold purple")) dn() print(Panel(" [bold white]EMAIL", subtitle='[bold purple]╭─────',subtitle_align='left',style="bold purple")) print(f" [bold purple]└──> [bold green]{email}") dn() print(Panel(" [bold white]PASSWORD", subtitle='[bold purple]╭─────',subtitle_align='left',style="bold purple")) print(f" [bold purple]└──> [bold green]{password}") dn() print(Panel(f"[bold white] FILLING CREDENTIALS [bold green1]DONE ",style="bold purple")) dn() print(Panel(f"[bold green] ACCOUNT LIVE",style="bold purple")) dn() print(Panel(" [bold white]DATE & TIME", subtitle='[bold purple]╭─────',subtitle_align='left',style="bold purple")) print(f" [bold purple]└──> [bold green]{tanggal} {waktu}") dn() try: code = 'code not found' except: code = 'code not found' if code: tree = Tree(Panel.fit("[bold green1] LOGIN SUCCESSFULLY ",style="bold purple")) tree.add(Panel.fit(f"[bold green1] STATUS [medium_purple1]➙ [bold green1]{status}",style="bold purple")) tree.add(Panel.fit(f"[bold green1] TIMESTAMP [medium_purple1]➙ [bold cyan1]{waktuu}",style="bold purple")) tree.add(Panel.fit(f"[bold green1] NAME [medium_purple1]➙ [bold green1]{first_name} {last_name}",style="bold purple")) tree.add(Panel.fit(f"[bold green1] UID [medium_purple1]➙ [bold green1]{id}",style="bold purple")) tree.add(Panel.fit(f"[bold green1] PASSWORD [medium_purple1]➙ [bold green1]{password}",style="bold purple")) tree.add(Panel.fit(f"[bold green1] BIRTHDAY [medium_purple1]➙ [bold green1]{birthday}",style="bold purple")) tree.add(Panel.fit(f"[bold green1] AGE [medium_purple1]➙ [bold green1]{age}",style="bold purple")) tree.add(Panel.fit(f"[bold green1] GENDER [medium_purple1]➙ [bold green1]{gender}",style="bold purple")) tree.add(Panel.fit(f"[bold green1] FBLINK [medium_purple1]➙ [bold green1]{url}",style="bold purple")) tree.add(Panel.fit(f"[bold green1] TOKEN [medium_purple1]➙ [bold green1]{token}",style="bold purple")) print(tree) open('/sdcard/AUTO-CREATE-JAHRA/create/toka.txt', 'a').write(token+'\n') open('/sdcard/AUTO-CREATE-JAHRA/create/tokaid.txt', 'a').write(id+'\n') open('/sdcard/AUTO-CREATE-JAHRA/create/NEWACCOUNT-ALIVE.txt', 'a').write(id+'|'+password+'|'+code+'\n') oks.append(id) os.system('espeak -a 300 "HEY, YOU, GOT, OK, id"') else: pass else: cps.append(id) #──────────────{ AUTO CREATE METHOD 2 }──────────────# def register_facebook_account2(password, first_name, last_name, birthday, age, gender, email): session = requests.Session() api_key = '882a8490361da98702bf97a021ddc14d' secret = '62f8ce9f74b12f84c123cc23437a4a32' accessToken = '350685531728|62f8ce9f74b12f84c123cc23437a4a32' req = { 'api_key': api_key, 'attempt_login': True, 'birthday': birthday.strftime('%Y-%m-%d'), 'client_country_code': 'US', 'fb_api_caller_class': 'com.facebook.registration.protocol.RegisterAccountMethod', 'fb_api_req_friendly_name': 'registerAccount', 'firstname': first_name, 'format': 'json', 'gender': gender, 'lastname': last_name, 'email': email, 'locale': 'en_US', 'method': 'user.register', 'password': password, 'reg_instance': generate_random_string(32), 'return_multiple_errors': True } sorted_req = sorted(req.items(), key=lambda x: x[0]) sig = ''.join(f'{k}={v}' for k, v in sorted_req) ensig = hashlib.md5((sig + secret).encode()).hexdigest() req['sig'] = ensig api_url = 'https://b-api.facebook.com/method/user.register' headers = { 'User-Agent': ua_bgraph, } proxy = get_random_proxy() response = requests.post(api_url, data=req, headers=headers, proxies=proxy, timeout=30) reg = response.json() id = reg.get('new_user_id') token = reg.get    sessionx=requests.Session()
