...
def viewmail():
  if os.path.exists("core/domain.txt"):
    with open("core/domain.txt","r") as cmmail:
      cmmail = cmmail.read()
    print(f"{acl}>>>{bcl} CURRENT _ MAIL{acl} :{ycl} {cmmail}")  
    x_X=input(f"\n\n{ycl} BACK MAIN MENU {acl}[{gcl}y{ncl}/{rcl}n{acl}] {acl}:{bcl} ")
    if x_X=="y":
      main()
    elif x_X=="n":
      ext()  
...
def coustomnewdomains():
  ...
  email_name=str(input(f"{acl} >>>{gcl} ENTER CUSTOM USERNAME  {acl}:{ycl} "))
  ...
  if domainsrow+1 >= ma:
    ...
    if domaincon:
      logop(f"{acl}[{gcl} !{acl} ] {ycl}SUCCESSFULLY CREATED A NEW EMAIL {acl}[{gcl} !{acl} ]")
    ...
    print(f"{acl}[{rcl} !{acl} ]{rcl} SELECTED DOMAIN NOT FOUND {acl}[{rcl} !{acl} ]")
...
def historylog():
  ...
  print(f"{acl}[{rcl} 0. {acl}]{pcl} BACK MAIN MENU {acl}[ {gcl}../{acl} ]")  
  in__=int(input(f"\n{bcl}>>>> {ycl}"))
      ...
      if str(defaultbox)==str(inboxget):
        ...
        if inboxget == []:
          ...
          if inboxget["message"]=="Email not found":
            os.system("clear;")
            print("{acl}[{rcl} !{acl} ] {ycl}YOUR MAIL HAS EXPIRED {acl}[{rcl} !{acl} ]".format(acl=acl,rcl=rcl,ycl=ycl))
            time.sleep(2)
            main2()
            break
          else:
...
def more():
  ...
  {acl}[{gcl} 2{bcl}.{acl} ]{ycl} CREATE CUSTOM MAIL{acl} [{bcl} ANY USERNAME {acl}]
  {acl}[{gcl} 3{bcl}.{acl} ]{ycl} SEE ALL EMAIL {acl}[{bcl} YOU CREATED {acl}]
  {acl}[{gcl} 4{bcl}.{acl} ]{ycl} VIEW CURRENT MAIL {acl}[ {ccl}ALL MAILS {acl}]
  {acl}[{gcl} 5{bcl}.{acl} ]{ycl} EMAILS HISTORY {acl}[ {bcl}YOU CAN LOG IN {acl}]
  {acl}[{gcl} 6{bcl}.{acl} ]{ycl} FB {acl}[{bcl} ZK VOLKSCHEV {acl}]
...
def more2():
  ...
  {acl}[{gcl} 5{bcl}.{acl} ]{ycl} VIEW CURRENT MAIL {acl}[ {ccl}ALL MAILS {acl}]
  {acl}[{gcl} 6{bcl}.{acl} ]{ycl} EMAILS HISTORY {acl}[ {bcl}YOU CAN LOG IN {acl}]
  {acl}[{gcl} 7{bcl}.{acl} ]{ycl} FACEBOOK {acl}[{bcl} ZK VOLKSCHEV {acl}]
  {acl}[{rcl} 0{bcl}.{acl} ]{rcl} EXIT {acl}[{bcl} ×××{acl} ]
  """.format(acl=acl,bcl=bcl,gcl=gcl,ycl=ycl,rcl=rcl,ccl=ccl)
  try:
    ...
    x_X=input(f"\n\n{ycl} BACK MAIN MENU OR TYPE {rcl}n{ycl} TO EXIT {acl}[{gcl}y{ncl}/{rcl}n{acl}] {acl}:{bcl} ")
    if x_X=="y":
      main2()
    elif x_X=="n":
      ext()      
def main2():
  try:
    ...
    {acl}[{gcl} 5{bcl}.{acl} ]{ycl} FACEBOOK {acl}[{bcl} ZK VOLKSCHEV {acl}]
...
def main():
  try:
    ...
    {acl}[{gcl} 4{bcl}.{acl} ]{ycl} FACEBOOK {acl}[{bcl} ZK VOLKSCHEV {acl}]
    {acl}[{rcl} 0{bcl}.{acl} ]{rcl} EXIT{acl} [{rcl} ××× {acl}]
    """.format(acl=acl,bcl=bcl,gcl=gcl,ycl=ycl,rcl=rcl,ccl=ccl)
    while True:
